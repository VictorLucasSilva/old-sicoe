...
def get_redirect_url_by_group(request):
    ...
    if user.groups.filter(name='Sem Acesso').exists():
        messages.warning(request, "Seu usuário não possui autorização para entrar no sistema. Email para solicitação de acesso (Com motivo e nome do estabelecimento): app-sicoe@bbts.com.br.")
        return 'login'
    ...
    messages.warning(request, "Seu usuário não possui perfil configurado. Email para solicitação de acesso (Com motivo e nome do estabelecimento): app-sicoe@bbts.com.br.")
    return 'login'
